...
const MENU_ITEMS = [
  { emoji: "🧳", label: "我的行程", desc: "查看所有已規劃的行程" },
  { emoji: "🏨", label: "收藏住宿", desc: "已加入的合法住宿清單" },
  { emoji: "❤️", label: "喜愛景點", desc: "探索地圖中收藏的地點" },
  { emoji: "🔔", label: "通知設定", desc: "行程提醒與優惠通知" },
  { emoji: "🌐", label: "語言設定", desc: "繁體中文 / English / 日本語" },
  { emoji: "🛡️", label: "隱私與安全", desc: "帳號安全與資料設定" },
];
  ...
  return (
    <div className="min-h-screen" style={{ background: CONIC }}>
      ...
      <div className="flex flex-col items-center pt-10 pb-6 px-4">
        ...
        <p className="text-[22px] font-bold text-[#141414]">旅行者</p>
        ...
        {MENU_ITEMS.map((item, i) => (
          <button key={item.label} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors text-left"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[15px] font-semibold text-[#141414]">{item.label}</p>
              <p className="text-[12px] mt-0.5 truncate" style={{ color: "#999" }}>{item.desc}</p>
            </div>
            <span className="text-[#ccc] text-[18px] flex-shrink-0">›</span>
          </button>
        ))}
      </div>
      ...
      <p className="text-center text-[13px] pb-6" style={{ color: "#ccc" }}>© 2026 Taibear · 個人設定即將推出更多功能</p>
    </div>
  );
}
